...
def _decision(
    ...
    if d_direction == "up" and noise_improvement is not None and noise_improvement < -4.0:
        warnings.append("D was raised and high-frequency noise got worse. Reduce or revert the D increase before adding more changes.")
    if d_direction == "down" and propwash_improvement is not None and propwash_improvement < -4.0:
        warnings.append("D was lowered and propwash-band error got worse. The D reduction may have removed too much damping.")
...
def build_tune_change_tracking(
    ...
    if after_clean:
        ...
        style_guidance = (
            "Do not move into Locked-In or Cinematic style tuning yet. Clean the remaining baseline issues first."
        )
